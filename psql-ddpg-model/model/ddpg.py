...
class DDPG(object):
    ...
    def _sample_batch(self):
        ...
        states = list(map(lambda x: x[0], batch))  # pylint: disable=W0141
        actions = list(map(lambda x: x[1], batch))  # pylint: disable=W0141
        rewards = list(map(lambda x: x[2], batch))  # pylint: disable=W0141
        next_states = list(map(lambda x: x[3], batch))  # pylint: disable=W0141
    ...
    def add_sample(self, state, action, reward, next_state):
        ...
        batch_state = self.totensor([state])
        batch_next_state = self.totensor([next_state])
    ...
    def choose_action(self, states):
        self.actor.eval() 
        act = self.actor(self.totensor([list(states)])).squeeze(0)
        self.actor.train()
